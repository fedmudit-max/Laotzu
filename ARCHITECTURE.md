...
OS-scheduled check-in: **`KingReminder` plugin** wrapping AlarmManager **`setAlarmClock()`** (primary — on-time through Doze, no user prompt for exact alarms). Fallback only if AlarmClock fails: **`setExactAndAllowWhileIdle()`** (needs `SCHEDULE_EXACT_ALARM` when granted) then inexact `setAndAllowWhileIdle()`. No `USE_EXACT_ALARM` / `WAKE_LOCK` / `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` in v1 — do not prompt for exact-alarm or battery Unrestricted during normal setup; reconsider battery exemption only if real-device testing shows delays/drops. If notifications are denied, open app notification settings (required for a visible banner). Fires with the app open, backgrounded, locked, or closed; reboot / date change reschedules via `BOOT_COMPLETED` + `DATE_CHANGED` (`ReminderBootReceiver` is `exported="false"`; system broadcasts still arrive). Force-stop clears alarms until the next open (resume reschedules). If today’s chosen time has passed and today is **not** logged and **not** already reminded, fire shortly — do not roll to tomorrow (that dropped tonight’s alarm when King was opened after 10:30). If today is already logged, skip and schedule tomorrow. Notification channel `king_daily_reminder_v3` uses **`IMPORTANCE_HIGH`** (heads-up + sound + vibration). Persist `{enabled, hour, minute}` in `kingReminder` (JS) and matching native SharedPreferences. Native also stores the last logged wall date. JS syncs that date from `dailyLog` via `setLoggedDate`. **At most one daily notification per calendar day** (`notifiedDate`); ignoring it does not send another until tomorrow. Changing the time to a **later time today** clears today’s `notifiedDate` so it can fire again. The in-app 2-minute test is separate.
